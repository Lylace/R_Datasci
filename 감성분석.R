#감성 분석
#특정 대상에 대하여 소비자가 느끼는 감성(긍정, 부적)과
#그에 대한 이유를 분석
...
#lapply 실행 결과 중 3행까지 출력
head(apple.text,3)

#감정분석을 위한 긍정/부정 사전을 불러옴
...
score.sentiment <- function(sentences, pos.words, neg.words, .progress='none')
{
  ...
  #감정기반 분석 후 점수 산출하는 함수
  scores <- laply(sentences, function(sentence, pos.words, neg.words){
    
    #의미없는 단어(문장부호, 기호 등) 제거
