...
sentence <- "분석할 문장에 포함된 단어들이 사전에 알맞는 형태(품사)로 정의 되어야 정확한 분석 가능"
extractNoun(sentence)

#SimplePos09 : 가장 상위 분류인 9개 품사로 분류
#SimplePos22 : 22개의 품사로 분류
...
#트윗글 읽어오기
tweets <- readLines('c:/Java/speech.txt')
class(tweets)
head(tweets, 3)
...
docs <- str_replace_all(tweets, "\\[ㄱ-ㅎ]","")
...
#dplyr 패키지를 이용해서 작업
#df_word의 내용을 정렬한 뒤 상위 20개만 추려냄
top20 <- df_word %>%
  arrange(desc(freq)) %>%
  head(20)
...
pal <- brewer.pal(8, "Dark2")   #Dark2 색상 중 8개 색 출력

wordcloud(words = df_word$word,    #단어
          freq = df_word$freq,     #빈도
          min.freq = 2,            #최소 단어 빈도
          max.words = 250,         #표현 단어수 
          random.order = F,        #고빈도 중앙배치 여부
          rot.per = .1,            #회전 단어 비율
